...
import { ChevronLeft, Printer, XCircle, CheckCircle, TruckIcon, RotateCcw } from 'lucide-react';
...
const OrderDetail = () => {
  ...
  const handleStatusUpdate = async (newStatus) => {
    try {
      ...
      // Sau khi thành công, cập nhật state
    ...
    <div className="min-h-screen bg-gray-50 p-6">
      ...
      <div className="grid grid-cols-2 gap-6 mb-6">
        ...
        <div className="bg-white rounded-lg shadow-sm p-6">
          ...
          <div className="space-y-2">
            <p><span className="text-gray-500">Phương thức:</span> {order.shipping.method}</p>
            <p><span className="text-gray-500">Phí vận chuyển:</span> {formatPrice(order.shipping.fee)}</p>
            <p><span className="text-gray-500">Dự kiến giao:</span> {new Date(order.shipping.estimatedDelivery).toLocaleDateString()}</p>
