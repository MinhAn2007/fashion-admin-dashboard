...
const EditVoucherModal = ({ isOpen, onClose, onSubmit, voucherData }) => {
  ...
  useEffect(() => {
    if (voucherData) {
      ...
      const { total_discount, total_uses, status, ...rest } = voucherData;
      setVoucher({
        ...rest,
        coupon_start_date: formatDate(voucherData.coupon_start_date),
        coupon_end_date: formatDate(voucherData.coupon_end_date),
      });
    }
  }, [voucherData]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_ENDPOINT}/api/voucher/${voucherData.id}`,
        {
          method: "PUT",
          headers: {
