  ...
  useEffect(() => {
    const fetchParentCategories = async () => {
      try {
        ...
        const filteredCategories = data.categories.filter(cat => cat.id !== category?.id) || [];
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_ENDPOINT}/api/categories/${category.id}`,
        {
          method: "PUT",
          headers: {
    ...
    <div className="fixed inset-0 z-[999999999] flex items-center justify-center">
      ...
      >
        <div className="flex items-center justify-between p-4 border-b">
          <h3 className="text-xl font-semibold text-gray-900">Chỉnh sửa danh mục</h3>
          <button
            onClick={onClose}
