...
const EditSkuModal = ({ isOpen, onClose, productId, skuId, initialData }) => {
  ...
  const [formData, setFormData] = useState({
    ...
    image: null,
    imagePreview: null,
    currentImage: null, // để lưu URL ảnh hiện tại
  });
  ...
  const uploadToS3 = async (file, userId, fileType = "image") => {
    try {
      ...
      const response = await fetch(
        `${API_ENDPOINT}/api/uploadAvatarS3/${userId}`,
        {
          method: "POST",
          body: formData,
        }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Upload image to S3 if a new image is selected
      ...
      const response = await fetch(
        `${API_ENDPOINT}/api/product/sku/${skuId}`,
        {
          method: "PUT",
          headers: {
    ...
    <div className="fixed inset-0 z-[999999999] flex items-center justify-center">
      ...
      >
        <div className="flex items-center justify-between p-4 border-b">
          <h3 className="text-xl font-semibold text-gray-900">Chỉnh sửa SKU</h3>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6">
            ...
            <div className="col-span-2">
              ...
              {(formData.imagePreview || formData.currentImage) && (
                <img
                  ...
                  alt="Preview"
                  className="mt-2 rounded h-auto w-96 mx-auto"
                />
              )}
            </div>
          </div>
          ...
          <div className="flex justify-end space-x-4 mt-6">
            ...
            <button
              ...
              disabled={loading}
            >
              {loading ? "Đang xử lý..." : "Cập nhật SKU"}
            </button>
          </div>
        </form>
      </div>
